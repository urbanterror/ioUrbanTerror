...
#ifdef BATTLEYE
		Info_SetValueForKey( info, "battleye", va("%i", cl_battleye->integer ) );
#endif
...
#ifdef BATTLEYE
		clc.sv_battleye = atoi( Cmd_Argv(1) );
#endif
		return;
	}
...
void CL_Frame ( int msec ) {
	...
	if( cls.state == CA_ACTIVE )
	{
		...
		else if( clc.sv_battleye )
			CL_BE_Load();
	}
	else if( clbe.module )
	{
		CL_BE_Unload();
		MSG_Clear( &cls.be.outMsg );
	}
#endif
...
#ifdef BATTLEYE
	MSG_InitOOB( &cls.be.outMsg, cls.be.outMsgBuf, sizeof(cls.be.outMsgBuf) );
	clbe.module = NULL;
#endif
...
#ifdef BATTLEYE
	cl_battleye = Cvar_Get( "cl_battleye", "1", CVAR_ARCHIVE );
#endif
