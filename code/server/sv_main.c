...
#ifdef BATTLEYE
	Info_SetValueForKey( infostring, "punkbuster", va("%i", sv_battleye->integer ? 2 : 0 ) );
#endif
	...
	if( svbe.module )
	{
		...
		for( i = 0; i < sv_maxclients->integer; i++ )
		{
			if( svs.clients[i].state == CS_ACTIVE && !(svs.clients[i].gentity && (svs.clients[i].gentity->r.svFlags & SVF_BOT))
				&& svs.clients[i].be.activationTime && (svs.time - svs.clients[i].be.activationTime) >= 1000 )
				{
					svbe.AddPlayer( i, svs.clients[i].name );
					svs.clients[i].be.activationTime = 0;
				}
		}
		if( !svbe.Run() )
		{
